...
public class HistoryByStoreTestSteps {
	...
	public void fetchGrandTotalForMonth(String month) throws IOException {
		...
		for (int i = 1; i < rowCount - 1; i++) {
			...
			grandTotalOnUi = Double.parseDouble(historyViewByStorePage.getStoresTableCellValue(rowCount, requiredColumn)
					.getText().replaceAll("[$,]", "").toString());
		}
		System.out.println("Grand total is: " + grandTotal);
		System.out.println("Grand total on UI is: " + grandTotalOnUi);

		if (!(Math.abs(grandTotalOnUi - grandTotal) == 0.00)) {
			System.out.println("\u001B[31m***********************Testcase is failed for " + month
					+ "***********************\u001B[0m");
			failedMonths.add(month);
		} else {
			System.out.println("\u001B[32m***********************Testcase is Passed for " + month
					+ "***********************\u001B[0m");
		}
		...
		softAssert.assertEquals(grandTotalOnUi, grandTotal, 0.001,
				"\u001B[31mAssertion Error on\u001B[0m " + failedMonths);
	}
